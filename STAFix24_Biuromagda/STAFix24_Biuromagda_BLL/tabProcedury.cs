...
namespace BLL
{
    public class tabProcedury
    {
        ...
        /// <returns></returns>
        public static int Update(SPWeb web, string procName)
        {
            SPList list = web.Lists.TryGetList(targetList);
            //if (list!=null)
            //{
                ...
                {
                    string statusP = item["enumStatusProcedury"] != null ? item["enumStatusProcedury"].ToString() : string.Empty;
                    if (statusP=="Zatwierdzona")
                    {
                        return item.ID;
                    }
                }
